...
$reqTipo = isset($_GET['tipo']) ? $_GET['tipo'] : '';
$reqIdPrestamo = isset($_GET['id_prestamo']) ? (int)$_GET['id_prestamo'] : 0;

$startPath = '';

if ($reqTipo === 'personales') {
    $startPath = 'Documentos personales';
} elseif ($reqTipo === 'prestamo' && $reqIdPrestamo > 0) {
    $sqlNum = "SELECT numero_prestamo_cliente FROM cliente_prestamo WHERE id_prestamo = ? LIMIT 1";
    $stNum = $conn->prepare($sqlNum);
    $stNum->bind_param("i", $reqIdPrestamo);
    $stNum->execute();
    $resNum = $stNum->get_result()->fetch_assoc();
    $stNum -> close();

    if ($resNum && isset ($resNum['numero_prestamo_cliente'])) {
      $numSecuencial = $resNum['numero_prestamo_cliente'];
    } else {
      $numSecuencial = $reqIdPrestamo;
    }
    $startPath = 'PRESTAMO_' . $numSecuencial;
}
...
if ($startPath !== '') {
  $fullStartPath = $folderPath . '/' . $startPath;
  if (!is_dir($fullStartPath)) {
    $startPath = '';
  }
}
...
<body>
  ...
  <script>
    (function () {
      ...
      let currentPath = normalizePath(data.inicial || '');
      let searchTerm = '';
      ...
      function renderList() {
        ...
        if (!filtered.length) {
          if(!term){
            $list.innertHTML = '<div class="empty-msg">Carpeta vacía.</div>';
          } else {
