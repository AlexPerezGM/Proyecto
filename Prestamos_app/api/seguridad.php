...
try {
  if ($action === 'permisos_list') {
    // La tabla en el esquema se llama `permisos` y las columnas contienen nombre_permiso
    $sql = "SELECT id_permiso, LOWER(REPLACE(nombre_permiso, ' ', '_')) AS clave, nombre_permiso AS nombre FROM permisos ORDER BY id_permiso";
    $res = $conn->query($sql)->fetch_all(MYSQLI_ASSOC);
    echo j(true, $res); exit;
  }

  if ($action === 'roles_list') {
    // Usar nombres de tablas del esquema: roles, roles_permisos, permisos
  ...
  if ($action === 'role_create') {
    ...
    $perms  = $_POST['permisos'] ?? []; // array de claves generadas en el frontend
    if ($nombre === '') { echo j(false,null,'Nombre de rol requerido'); exit; }

    // La tabla se llama `roles` y requiere nombre_rol y descripcion (no null). Usamos descripcion = nombre por defecto.
    $descripcion = $nombre;
    ...
    if (!empty($perms)) {
      // traducir claves -> id_permiso usando LOWER(REPLACE(nombre_permiso,' ','_')) para coincidir con la clave enviada
      $escaped = array_map(fn($s) => "'".$conn->real_escape_string($s)."'", $perms);
      $in = implode(',', $escaped);
      $map = $conn->query("SELECT id_permiso FROM permisos WHERE LOWER(REPLACE(nombre_permiso,' ', '_')) IN ($in)")->fetch_all(MYSQLI_ASSOC);
      foreach ($map as $m) {
  ...
  if ($action === 'role_get') {
    $idRol = (int)($_POST['id_rol'] ?? 0);
    $rol = $conn->query("SELECT id_rol, nombre_rol AS nombre FROM roles WHERE id_rol=$idRol")->fetch_assoc();
    $perms = $conn->query("SELECT LOWER(REPLACE(p.nombre_permiso,' ', '_')) AS clave FROM roles_permisos rp JOIN permisos p ON p.id_permiso=rp.id_permiso WHERE rp.id_rol=$idRol")->fetch_all(MYSQLI_ASSOC);
  ...
  if ($action === 'users_list') {
    // Combinar usuario con usuarios_roles y roles (esquema usa usuarios_roles y roles.nombre_rol)
  ...
  if ($action === 'empleados_list') {
    ...
    $res = $conn->query($sql)->fetch_all(MYSQLI_ASSOC);
    echo j(true, $res);  // ← DEVUELVE el resultado
  ...
  if ($action === 'user_create') {
    ...
    if ($user==='' || $pass==='' || $idRol<=0 || $idDatos<=0) { 
      echo j(false,null,'Datos requeridos'); exit; }

    $hash = password_hash($pass, PASSWORD_BCRYPT);
    $stmt = $conn->prepare("INSERT INTO usuario (nombre_usuario, contrasena, id_datos_persona) VALUES (?, ?, ?)");
  ...
  if ($action === 'user_get') {
    ...
    $u = $conn->query("SELECT id_usuario, nombre_usuario, id_datos_persona FROM usuario WHERE id_usuario=$id")->fetch_assoc();
    $roles = $conn->query("SELECT id_rol FROM usuarios_roles WHERE id_usuario=$id")->fetch_all(MYSQLI_ASSOC);
  ...
  if ($action === 'user_update') {
    ...
    $idRol = (int)($_POST['id_rol'] ?? 0);
    $idDatos = (int)($_POST['id_datos_persona'] ?? 0);
    if ($id<=0 || $user==='' || $idRol<=0 || $idDatos<=0) { echo j(false,null,'Datos incompletos'); exit; }
  ...
  echo j(false, null, 'Error en el servidor: '.$e->getMessage()); exit;
}