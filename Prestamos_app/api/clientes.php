...
function upload_doc_cliente(mysqli $conn) {
    ...
    $tipoSan   = san_nombre_archivo($tipo_archivo);          
...
function resolve_id_documentacion_cliente(mysqli $conn, string $tipoCodigo): int {
    ...
    if ($tipoCodigo === 'CRONOGRAMA')  $patterns[] = 'CRONOGRAMA';
    ...
    foreach ($cols as $field) {
        foreach ($patterns as $pat) {
            $pat = strtoupper($pat);
            $sql = "SELECT id_documentacion_cliente 
            FROM cat_documentacion_cliente WHERE UPPER($field) LIKE CONCAT('%', ?, '%') LIMIT 1";
            $st  = $conn->prepare($sql);
    ...
    if ($id <= 0) {
        $rs2 = $conn->query("SELECT id_documentacion_cliente 
        FROM cat_documentacion_cliente 
        ORDER BY id_documentacion_cliente 
        LIMIT 1");
...
try {
    ...
    if ($action === 'create') {
        ...
        if (!$mayor) 
            bad('El cliente debe ser mayor de edad', 422);
    ...
    if ($action === 'update') {
        ...
        if ($estadoCliente !== '') {
            $st = $conn->prepare("UPDATE datos_persona SET nombre=?, apellido=?, fecha_nacimiento=?, genero=?, estado_cliente=? 
            WHERE id_datos_persona=?");
        ...
        if ($direccion_alguno) {
            if (!$direccion_completa) 
                bad('Para actualizar dirección debes enviar: ciudad, sector, calle y numero_casa.', 422);
            $conn->query("DELETE FROM direccion WHERE id_datos_persona=".$id_dp);
        ...
        if ($ing>0 || $egr>0) {
            $conn->query("DELETE fi FROM fuente_ingreso fi JOIN ingresos_egresos ie 
            ON fi.id_ingresos_egresos=ie.id_ingresos_egresos WHERE ie.id_cliente=".$id);
            ...
            bind_params_safe($st,"idd",[$id,$ing,$egr]); 
            $st->execute(); 
            $id_ie = $conn->insert_id; 
            $st->close();
    ...
    if ($action === 'delete') {
        ...
        $st->close();
        if ($t>0) 
            bad('No se puede eliminar: el cliente tiene préstamos asociados.', 409);
