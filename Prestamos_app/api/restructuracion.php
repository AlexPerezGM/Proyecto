...
if ($action === 'get') {
  ...
  if (!$id_prestamo) 
    j(false, [], ['msg'=>'id_prestamo requerido']);
  ...
  $stmt->close();
  if (!$p) 
    j(false, [], ['msg'=>'Préstamo no encontrado']);
  ...
  if ($tasa===null) {
    ...
    $stmt->close();
    if ($tp){
       $tasa = (float)$tp['tasa_interes']; 
       $tipo_amort_id = (int)$tp['id_tipo_amortizacion']; 
       $tipo_amort = $tp['tipo_amortizacion']; 
      }
  }
  $sql = "
  ...
  $stmt->bind_param('i', $id_prestamo);
  $stmt->execute(); 
  $cuo = $stmt->get_result()->fetch_assoc(); 
  $stmt->close();
...
if ($action === 'update') {
  ...
  if(!$id_prestamo) 
    j(false,[],['msg' => 'id_prestamo requerido']);
  if($nuevo_plazo <= 0) 
    j(false,[],['msg' => 'Nuevo plazo invalido']);
  ...
  if (!$cond) 
    j(false, [],['msg'=>'No se pudo cargar la condicion actual']);
    $tasa_actual = isset($cond['tasa_interes']) ? (float)$cond['tasa_interes'] : 0.0;
    $tipo_amortizacion = isset($cond['id_tipo_amortizacion']) ? (int)$cond['id_tipo_amortizacion'] : 0;
    $periodo = isset($cond['id_periodo_pago']) ? (int)$cond['id_periodo_pago'] : 0;
