...
try {
  ...
  // Verificar permisos de administrador (simplificado por ahora)
  session_start();
...
function handleDeduccion($conn, $action) {
  switch($action) {
    case 'list':
      $stmt = $conn->prepare("SELECT id_tipo_deduccion, tipo_deduccion, valor FROM cat_tipo_deduccion ORDER BY tipo_deduccion");
      $stmt->execute();
    ...
    case 'save':
      ...
      if (empty($tipo)) bad('El tipo de deducción es requerido');
      if ($valor < 0 || $valor > 100) bad('El valor debe estar entre 0 y 100');
      ...
      if ($id > 0) {
        ...
        // Crear
        $stmt = $conn->prepare("INSERT INTO cat_tipo_deduccion (tipo_deduccion, valor) VALUES (?, ?)");
    ...
    case 'delete':
      ...
      $stmt->execute();
      if ($stmt->affected_rows === 0) bad('No se encontró el registro a eliminar');
...
function handleMoneda($conn, $action) {
  switch($action) {
    case 'list':
      $stmt = $conn->prepare("SELECT id_tipo_moneda, tipo_moneda, valor FROM cat_tipo_moneda ORDER BY tipo_moneda");
      $stmt->execute();
      $result = $stmt->get_result();
      $data = [];
      while ($row = $result->fetch_assoc()) {
    ...
    case 'save':
      ...
      if (empty($tipo)) bad('El tipo de moneda es requerido');
      if ($valor < 0) bad('El valor debe ser mayor a 0');
...
function handleContrato($conn, $action) {
  switch($action) {
    case 'list':
      $stmt = $conn->prepare("SELECT id_tipo_contrato, tipo_contrato FROM cat_tipo_contrato ORDER BY tipo_contrato");
      $stmt->execute();
      $result = $stmt->get_result();
      $data = [];
      while ($row = $result->fetch_assoc()) {
    ...
    case 'save':
      ...
      if (empty($tipo)) bad('El tipo de contrato es requerido');
...
function handleGarantia($conn, $action) {
  switch($action) {
    case 'list':
      $stmt = $conn->prepare("SELECT id_tipo_garantia, tipo_garantia, descripcion FROM cat_tipo_garantia ORDER BY tipo_garantia");
      $stmt->execute();
      $result = $stmt->get_result();
      $data = [];
      while ($row = $result->fetch_assoc()) {
    ...
    case 'save':
      ...
      if (empty($tipo)) bad('El tipo de garantía es requerido');
...
function handleBeneficio($conn, $action) {
  switch($action) {
    case 'list':
      $stmt = $conn->prepare("SELECT id_beneficio, tipo_beneficio, valor FROM cat_beneficio ORDER BY tipo_beneficio");
      $stmt->execute();
      $result = $stmt->get_result();
      $data = [];
      while ($row = $result->fetch_assoc()) {
    ...
    case 'save':
      ...
      if (empty($tipo)) bad('El tipo de beneficio es requerido');
      if ($valor < 0) bad('El valor debe ser mayor o igual a 0');
      ...
      if ($id > 0) {
        ...
        // También actualizar en configuracion si existe
        ...
        // También crear en configuracion
        $config_key = 'BENEFICIO_' . strtoupper(str_replace(' ', '_', $tipo));
    ...
    case 'delete':
      ...
      // Obtener el tipo de beneficio antes de eliminar
      ...
      if (!$beneficio) bad('No se encontró el beneficio');
      ...
      // También eliminar de configuracion
...
function handleConfiguracion($conn, $action) {
  switch($action) {
    case 'list':
      $stmt = $conn->prepare("SELECT id_configuracion, nombre_configuracion, valor_decimal, estado, actualizado_en FROM configuracion ORDER BY nombre_configuracion");
      $stmt->execute();
      $result = $stmt->get_result();
      $data = [];
      while ($row = $result->fetch_assoc()) {
    ...
    case 'save':
      ...
      if (empty($nombre)) bad('El nombre de la configuración es requerido');
      if (!in_array($estado, ['Activo', 'Inactivo'])) bad('Estado inválido');

      if ($id > 0) {
        // Actualizar
        $stmt = $conn->prepare("UPDATE configuracion SET nombre_configuracion = ?, valor_decimal = ?, estado = ? WHERE id_configuracion = ?");
...
function handleMora($conn, $action) {
  switch($action) {
    case 'list':
      $stmt = $conn->prepare("SELECT id_mora, fecha_registro, dias_gracia, porcentaje_mora, vigente_desde, vigente_hasta, estado FROM config_mora ORDER BY vigente_desde DESC");
      $stmt->execute();
      $result = $stmt->get_result();
      $data = [];
      while ($row = $result->fetch_assoc()) {
    ...
    case 'save':
      ...
      if ($porcentaje_mora < 0 || $porcentaje_mora > 100) bad('El porcentaje de mora debe estar entre 0 y 100');
      if ($dias_gracia < 0) bad('Los días de gracia no pueden ser negativos');
      if (empty($vigente_desde)) bad('La fecha vigente desde es requerida');
      if (!in_array($estado, ['Activo', 'Inactivo'])) bad('Estado inválido');

      if ($id > 0) {
        // Actualizar
        $stmt = $conn->prepare("UPDATE config_mora SET dias_gracia = ?, porcentaje_mora = ?, vigente_desde = ?, vigente_hasta = ?, estado = ? WHERE id_mora = ?");
        ...
        $stmt = $conn->prepare("INSERT INTO config_mora (fecha_registro, dias_gracia, porcentaje_mora, vigente_desde, vigente_hasta, estado) VALUES (CURDATE(), ?, ?, ?, ?, ?)");
...
function handleTipoPrestamo($conn, $action) {
  switch($action) {
    ...
    case 'save':
      ...
      if ($id > 0) {
        // Actualizar
        $stmt = $conn->prepare("UPDATE tipo_prestamo SET nombre = ?, tasa_interes = ?, monto_minimo = ?, id_tipo_amortizacion = ?, plazo_minimo_meses = ?, plazo_maximo_meses = ? WHERE id_tipo_prestamo = ?");
        // Tipos: s (nombre), d (tasa_interes), d (monto_minimo), i (id_tipo_amortizacion), i (plazo_minimo), i (plazo_maximo), i (id)
...
function handleAuditoria($conn, $action) {
  switch($action) {
    ...
    case 'detail':
      ...
      if (!$data) bad('Registro de auditoría no encontrado');
...
function handleFondos($conn, $action) {
  switch($action) {
    case 'list':
      $stmt = $conn->prepare("SELECT id_fondo, cartera_normal, cartera_vencida, total_fondos, actualizacion FROM fondos ORDER BY actualizacion DESC LIMIT 1");
      $stmt->execute();
      $result = $stmt->get_result();
    ...
    case 'initialize':
      ...
      if ($monto_inicial <= 0) bad('El monto inicial debe ser mayor a 0');
    ...
    case 'aporte_directo':
      $monto = fnum_in('monto');
      if ($monto <= 0) bad('El monto debe ser mayor a 0');

      // Obtener/crear registro de fondos
      $stmt = $conn->prepare("SELECT id_fondo, cartera_normal, cartera_vencida, total_fondos FROM fondos ORDER BY actualizacion DESC LIMIT 1");
      $stmt->execute();
      $result = $stmt->get_result();
      $fondos = $result->fetch_assoc();
      if (!$fondos) {
      ...
      // Registrar movimiento sin caja (NULL)
      $stmt = $conn->prepare("INSERT INTO movimiento_fondo (id_fondo, id_caja, monto, tipo_movimiento) VALUES (?, NULL, ?, 'aporte')");
    ...
    case 'movimiento':
      ...
      if ($monto <= 0) bad('El monto debe ser mayor a 0', 400);
      if (!in_array($tipo, ['aporte', 'desembolso'])) bad('Tipo de movimiento inválido', 400);
      if ($id_caja <= 0) bad('Debe seleccionar una caja válida', 400);

      // Verificar que la caja esté abierta y obtener monto disponible
      $stmt = $conn->prepare("SELECT estado_caja, monto_asignado FROM caja WHERE id_caja = ?");
      $stmt->bind_param('i', $id_caja);
      $stmt->execute();
      $result = $stmt->get_result();
      $caja = $result->fetch_assoc();
      if (!$caja || $caja['estado_caja'] !== 'Abierta') bad('La caja seleccionada no está disponible', 400);
      if ($tipo === 'desembolso' && $caja['monto_asignado'] < $monto) bad('La caja no tiene saldo suficiente para el desembolso', 400);
      ...
      $stmt = $conn->prepare("SELECT cartera_normal, cartera_vencida, total_fondos FROM fondos WHERE id_fondo = ?");
      $stmt->bind_param('i', $id_fondo);
      $stmt->execute();
      $result = $stmt->get_result();
      $totales = $result->fetch_assoc();
      if (!$totales) bad('Registro de fondos no encontrado');

      if ($tipo === 'aporte') {
        // Aporte aumenta los fondos generales (entrada externa)
        $totales['cartera_normal'] += $monto;
        $totales['total_fondos'] += $monto;
        // Opcional: si se desea reflejar en caja, se podría sumar a monto_asignado (no solicitado explícitamente)
      } elseif ($tipo === 'desembolso') {
        // Desembolso: dinero sale del sistema -> reducir caja y fondos
        if ($totales['cartera_normal'] < $monto) bad('Fondos insuficientes para desembolso', 400);
        if ($totales['total_fondos'] < $monto) bad('Total de fondos insuficiente', 400);
        $totales['cartera_normal'] -= $monto;
        $totales['total_fondos'] -= $monto;
        // Reducir monto asignado en caja
        $nuevo_monto_caja = $caja['monto_asignado'] - $monto;
        if ($nuevo_monto_caja < 0) bad('Error interno: saldo de caja negativo', 500);
        $stmt = $conn->prepare("UPDATE caja SET monto_asignado = ? WHERE id_caja = ?");
        $stmt->bind_param('di', $nuevo_monto_caja, $id_caja);
        $stmt->execute();
      }

      // Actualizar registro fondos
      $stmt = $conn->prepare("UPDATE fondos SET cartera_normal = ?, total_fondos = ? WHERE id_fondo = ?");
...
function handleCaja($conn, $action) {
  switch($action) {
    ...
    case 'open':
      ...
      if ($id_empleado <= 0) bad('Debe seleccionar un empleado válido', 400);
      if ($monto_asignado < 0) bad('El monto asignado no puede ser negativo', 400);

      // Verificar que el empleado no tenga una caja abierta
      $stmt = $conn->prepare("SELECT id_caja FROM caja WHERE id_empleado = ? AND estado_caja = 'Abierta'");
      ...
      // Verificar fondos disponibles (solo para validar capacidad, sin descontar)
      $stmt = $conn->prepare("SELECT id_fondo, cartera_normal, total_fondos FROM fondos ORDER BY actualizacion DESC LIMIT 1");
      $stmt->execute();
      $result = $stmt->get_result();
      $fondos = $result->fetch_assoc();
      if (!$fondos) bad('Debe registrar fondos iniciales antes de abrir una caja', 400);
      if ($fondos['cartera_normal'] < $monto_asignado || $fondos['total_fondos'] < $monto_asignado) bad('Fondos insuficientes para asignar a la caja', 400);

      // Abrir nueva caja (asignación interna, no altera totales de fondos)
    ...
    case 'close':
      ...
      $stmt->bind_param('i', $id);
      $stmt->execute();
      if ($stmt->affected_rows === 0) bad('No se encontró la caja o ya está cerrada');
