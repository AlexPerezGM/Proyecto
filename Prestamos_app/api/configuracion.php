...
function handleFondos($conn, $action) {
  switch($action) {
    ...
    case 'initialize':
      ...
      if ($monto_inicial <= 0) bad('El monto inicial debe ser mayor a 0', 400);
    ...
    case 'retiro_directo':
      ...
      $stmt = $conn->prepare("INSERT INTO movimiento_fondo (id_fondo, id_caja, monto, tipo_movimiento) VALUES (?, NULL, ?, 'retiro')");
      $stmt->bind_param('id', $fondos['id_fondo'], $monto);
      $stmt->execute();
...
function handlePlantillaNotificacion($conn, $action) {
    switch ($action) {
      ...
      $descripcion = s('descripcion');
      $tipo = s('tipo');
            if (empty($nombre)) bad('El nombre de la plantilla es requerido');
            if (empty($tipo)) bad('El tipo de la plantilla es requerido');
            if ($id > 0) {
