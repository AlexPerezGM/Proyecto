...
try {
  ...
  $ing = fnum('ingresos_mensuales');
  $egr = fnum('egresos_mensuales');

  function validar_ingresos_egresos($ing, $egr) {
    if ($ing > 0 || $egr > 0) {
      if ($ing < 10000) bad('Ingresos mensuales deben ser ≥ 10,000.');
      if ($ing <= $egr) bad('Ingresos deben ser mayores que egresos.');
    } 
  }
  // -------- CREAR ----------
  ...
  try {
    ...
    validar_ingresos_egresos($ing, $egr);
    ...
    // 1) datos_persona (incluye estado_cliente)
    ...
    // 3) documento_identidad (numero_documento es UNIQUE)
    $st = $conn->prepare("
    ...
    // 5) dirección: insertar SOLO si viene completa (NOT NULL en la tabla)
    ...
    // 6) ingresos/egresos + fuente (opcional)
    $id_ie = null;
    if ($ing>0 || $egr>0) {
      ...
      if ($fuente!=='') {
        // OJO: la tabla fuente_ingreso NO tiene columna 'monto' en el esquema nuevo
    ...
    if ($conn) { try { $conn->rollback(); } catch (Throwable $e2) {} }
