...
if ($action === 'pay') {
  ...
  if ($cuota) {
    ...
    // bajar saldo_cuota y marcar pagada si llega a 0
    $nuevo_saldo = max(0.0, (float)$cuota['saldo_cuota'] - ($m_cargos + $m_interes + $m_capital));
    $estado = ($nuevo_saldo <= 0.0001) ? 'Pagada' : $cuota['estado_cuota'];
    $sql = "UPDATE cronograma_cuota SET saldo_cuota=?, estado_cuota=? WHERE id_cronograma_cuota=?";
    $st = $conn->prepare($sql);
