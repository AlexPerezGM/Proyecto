...
if ($action === 'pay'){
  $id_prestamo = num_or_null(rq('id_prestamo')); 
  if(!$id_prestamo) 
    bad('id_prestamo invalido');
  $metodo = (string)rq('metodo', ''); 
  if ($metodo === '') 
    bad('metodo requerido');
  $monto = (float)rq('monto', 0); 
  if ($monto <= 0) 
    bad('monto invalido');
  ...
  try {
    ...
    $id_cliente = null;
    $numero_documento = '';
    $numero_prestamo_cliente = null;
    $st_p = $conn->prepare("SELECT c.id_cliente, dp.nombre, dp.apellido, di.numero_documento,
                     cp.numero_prestamo_cliente
                FROM prestamo p
                JOIN cliente c ON c.id_cliente = p.id_cliente
                JOIN datos_persona dp ON dp.id_datos_persona = c.id_datos_persona
                LEFT JOIN documento_identidad di ON di.id_datos_persona = dp.id_datos_persona
                LEFT JOIN cliente_prestamo cp ON cp.id_prestamo = p.id_prestamo
    ...
    if ($st_p) {
      ...
      if ($r) {
        ...
        $numero_prestamo_cliente = isset($r['numero_prestamo_cliente']) ? (int)$r['numero_prestamo_cliente'] : null;
      }
      $st_p->close();
    }
    if ($id_cliente !== null) {
      ...
      $prestamoLabel = ($numero_prestamo_cliente !== null && $numero_prestamo_cliente > 0)
        ? ('PRESTAMO_' . $numero_prestamo_cliente)
        : ('PRESTAMO_' . $id_prestamo);
      $prestamoDir = $clienteDir . '/' . $prestamoLabel;
      ...
      $htmlFactura = "<!DOCTYPE html>\n<html lang=\"es\">\n<head>\n".
        ...
        "<p><strong>Número de préstamo:</strong> " . htmlspecialchars((string)($numero_prestamo_cliente ?? $id_prestamo), ENT_QUOTES, 'UTF-8') . "<br />\n".
...
if ($action === 'close') {
  ...
  if ($saldo > 0.0001) 
    bad('No se puede cerrar: aún existe saldo pendiente.');

  $sql = "SELECT id_estado_prestamo 
  FROM cat_estado_prestamo 
  WHERE estado='Cerrado' LIMIT 1";
  $st = $conn->prepare($sql);
  $st->execute();
