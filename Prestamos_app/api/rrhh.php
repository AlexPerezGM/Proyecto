...
function resolve_id_documentacion_empleado($conn, string $tipoCodigo){
  ...
  return (int)($rs2 ? $rs2->fetch_column() : 0);
    ...
    if ($action === 'upload_doc') {
        ...
        if ($id_empleado <= 0) 
          bad('id_empleado requerido', 400);

        $tipo = strtoupper(trim($_POST['tipo_archivo'] ?? ''));
        if ($tipo === '') 
          bad('Tipo de documento requerido', 422);
...
if ($action === 'options'){
  $gen = $conn->query("SELECT id_genero, genero 
  FROM cat_genero 
  ORDER BY id_genero")->fetch_all(MYSQLI_ASSOC);

  $con = $conn->query("SELECT id_tipo_contrato, tipo_contrato 
  FROM cat_tipo_contrato 
  ORDER BY id_tipo_contrato")->fetch_all(MYSQLI_ASSOC);
  ...
  $ded = $conn->query("SELECT id_tipo_deduccion, tipo_deduccion 
  FROM cat_tipo_deduccion 
  ORDER BY id_tipo_deduccion ")->fetch_all(MYSQLI_ASSOC);

  $ben = $conn->query("SELECT id_beneficio, tipo_beneficio 
  FROM cat_beneficio 
  ORDER BY id_beneficio")->fetch_all(MYSQLI_ASSOC);
...
if ($action === 'ajuste_list'){
  ...
  if(!$id_empleado) 
    j(['ok'=>false, 'msg'=> 'id_empleado requerido']);
...
if ($action === 'emp_create' || $action === 'emp_update'){
  ...
  try{
    if ($action === 'emp_create'){
      ...
      $st=$conn->prepare("SELECT id_datos_persona 
      FROM empleado 
      WHERE id_empleado=?"); $st->bind_param('i',$id_empleado); $st->execute();
      $rs=$st->get_result(); 
      $row=$rs->fetch_assoc(); 
      $st->close();
...
if ($action === 'nomina_preview' || $action === 'nomina_save'){
  $frecuencia = getStr('frecuencia', 'Mensual');
  $fecha_inicio = getStr('fecha_inicio');
  $fecha_fin = getStr('fecha_fin');
  $fecha_pago = getStr('fecha_pago');
  $id_emp = getStr('id_empleado');

  if(!$fecha_inicio || !$fecha_fin || !$fecha_pago)
    j(['ok'=>false,'msg'=>'Faltan campos requeridos']);
  $sqlCall = "CALL generar_nomina_empleado(?, ?, ?, ?)";
  $id_nomina = null;

  try {
    $st = $conn->prepare($sqlCall);
    $st->bind_param('ssss', $fecha_inicio, $fecha_fin, $fecha_pago, $frecuencia);

    if (!$st->execute()){
      throw new Exception('Error ejecutando nomina: '. $st->error);
    }
    ...
    if ($action === 'nomina_save'){
      j(['ok' => true, 'id_nomina'=>$id_nomina, 'msg'=>'Nomina generada y guardada correctamente']);
    }

    if(!$id_nomina) throw new Exception("No se genero ID de nomina");

    $filtroEmpleado = ($id_emp==='ALL' || !$id_emp ? '' : 'AND ne.id_empleado = ' . (int)$id_emp.' ');

    $sql = "SELECT ne.id_empleado, ne.salario_base, ne.salario_neto, CONCAT(dp.nombre,' ',dp.apellido) AS nombre
    ...
    foreach($rows as &$r){
      ...
      $idNErow = $conn->query("SELECT id_nomina_empleado FROM nomina_empleado WHERE id_nomina=$id_nomina AND id_empleado=$idEmpleado LIMIT 1")->fetch_assoc();
      $idNE = $idNErow ? (int)$idNErow['id_nomina_empleado'] : 0;

      $sqlHX = "SELECT SUM(monto_total) AS total FROM horas_extras WHERE id_empleado=$idEmpleado AND fecha BETWEEN '$fecha_inicio' AND '$fecha_fin'";
      $hxRow = $conn->query($sqlHX)->fetch_assoc();
      $r['horas_extra'] = ($hxRow && $hxRow['total']!==null) ? (float)$hxRow['total'] : 0.0;

      $benRow = $idNE ? $conn->query("SELECT SUM(monto) AS total FROM beneficios_empleado WHERE id_nomina_empleado=$idNE")->fetch_assoc() : null;
      $r['bonificaciones'] = ($benRow && $benRow['total']!==null) ? (float)$benRow['total'] : 0.0;

      $dedRow = $idNE ? $conn->query("SELECT SUM(monto) AS total FROM deducciones_empleado WHERE id_nomina_empleado=$idNE")->fetch_assoc() : null;
      $r['deducciones'] = ($dedRow && $dedRow['total']!==null) ? (float)$dedRow['total'] : 0.0;
    }
    unset($r);
    j(['ok'=>true, 'rows'=>$rows, 'id_nomina'=>$id_nomina]);
  } catch (Exception $e) {
    j(['ok'=>false, 'msg'=>'Error: ' .$e->getMessage() ]);
  }
}

if ($action === 'nomina_comprobantes'){
  $id_nomina = i('id_nomina');
  $id_empleado = getInt('id_empleado');

  $nomina = null;
  if ($id_nomina > 0){
    $rs = $conn->query("SELECT * FROM nomina WHERE id_nomina= $id_nomina");
    $nomina = $rs->fetch_assoc();
  }

  if (!$nomina) {
    $periodo = getStr('periodo');
    if ($periodo) {
      $rs = $conn->query("SELECT * FROM nomina WHERE periodo='" .$conn->real_escape_string($periodo)."' ORDER BY id_nomina DESC LIMIT 1");
      $nomina = $rs->fetch_assoc();
    }
  }

  if(!$nomina) j(['ok'=>false, 'msg'=>'Nomina no encontrada']);

  $id_nomina = $nomina['id_nomina'];
  $periodoStr = $nomina['periodo'];
  $frecuencia = $nomina['tipo_frecuencia'];
  $fechaI = date('d/m/Y', strtotime($nomina['fecha_inicio']));
  $fechaF = date('d/m/Y', strtotime($nomina['fecha_fin']));
  ...
  ob_start();
  echo "<html><head><meta charset='utf-8'><title>Comprobante $periodoStr</title>
  <style>
  ...
  foreach($rows as $r){
    ...
    AND fecha BETWEEN '{$nomina['fecha_inicio']} ' AND '{$nomina['fecha_fin']}'")->fetch_assoc();
    ...
    <div class='recibo'>
      <div class='header'>
        ...
        <div class='subtitle'>Periodo: $frecuencia | Periodo: $fechaI al $fechaF</div>
      </div>
      <div class='row'>
