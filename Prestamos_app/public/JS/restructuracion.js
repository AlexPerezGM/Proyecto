(() => {
  ...
  async function cargar(page=1){
    ...
    $totalTag.textContent = total + ' en cola';
  ...
  document.addEventListener('click', async (e) => {
    ...
    document.getElementById('nuevo_monto').value = (+json.data.monto_actual).toFixed(2);
  ...
  $frm.addEventListener('submit', async (e) => {
    ...
    const monto = parseFloat(fd.get('nuevo_monto') || '0');
    const plazo = parseInt(fd.get('nuevo_plazo') || '0', 10);
    if (!(monto>0) || !(plazo>0)) { alert('Verifica monto y plazo'); return; }
