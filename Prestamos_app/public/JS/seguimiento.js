(function () {
  ...
  const $btnAutoNotificacion = document.getElementById('btnAutoNotify');
  ...
  function showSpinnerMorosos(v) {
    if (!$spinnerMorosos)
      return;
    if (v) {
      $spinnerMorosos.hidden = false;
      $spinnerMorosos.setAttribute('aria-hidden', 'false');
    } else {
      $spinnerMorosos.hidden = true;
      $spinnerMorosos.setAttribute('aria-hidden', 'true');
    }
  }
  ...
  async function jsonFetchMorosos(body) {
    ...
    try {
      ...
      try {
        ...
        $errMorosos.hidden = false;
        $errMorosos.textContent = 'Respuesta no-JSON de la API:\n' + t.slice(0, 2000);
        throw parseErr;
      }
    } catch (e) {
      if ($errMorosos.hidden) {
        $errMorosos.hidden = false;
        $errMorosos.textContent = 'Error consultando API:\n' + (e.message || e);
      }
      throw e;
  ...
  $tabBtns.forEach(b => b.addEventListener('click', () => {
    ...
    if ($btnAutoNotificacion) {
      $btnAutoNotificacion.style.display = (tab === 'morosos') ? 'block' : 'none';
    }
    if (tab === 'morosos') {
      cargarMorosos(1);
      if (currentLoan) {
  ...
  async function decidir(decision) {
    ...
    if (!json.ok)
      return alert(json.msg || 'No se pudo registrar la decisión');
  ...
  function renderDocuments(docs) {
    if (!docs || !docs.length) {
      $evDocs.textContent = 'Sin documentos registrados.';
      return;
    }
  ...
  if ($btnCheckData) $btnCheckData.addEventListener('click', async () => {
    if (!currentLoan)
      return alert('Selecciona primero un préstamo en la lista.');
  ...
  if ($btnEnviarNotif) $btnEnviarNotif.addEventListener('click', async () => {
    ...
    try {
      ...
      try {
        ...
        data = { success: false, message: text.slice(0, 2000) };
      }
      if (!data.success)
        return alert(data.message || 'No se pudo enviar la notificación.');
  ...
  if ($btnExportMorosos) $btnExportMorosos.addEventListener('click', () => {
    ...
    const csv = toCSV(rows);
    if (!csv)
      return alert('No hay datos de morosos para exportar.');
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url;
  ...
  if ($btnAutoNotificacion) $btnAutoNotificacion.addEventListener('click', async () => {
    if (!confirm('Estas seguro de hacer el envio masivo de notificaciones?')) {
      return;
    }

    const text = $btnAutoNotificacion.textContent;
    $btnAutoNotificacion.disabled = true
    $btnAutoNotificacion.textContent = 'Enviando...'

    const url = ((typeof window !== 'undefined' && window.APP_BASE) ? window.APP_BASE : '/') + 'api/notificaciones_auto.php?action=run';
    ...
    try {
      ...
      if (data.ok) {
        let msg = data.msg || 'Proceso finalizado.';
        if (data.resumen) {
          msg += `\n\nResumen:\n- Pre-mora enviados: ${data.resumen.pre_mora_enviadas}\n- Mora aplicada enviadas: ${data.resumen.mora_enviadas}`;
        }
        alert(msg);
        cargarMorosos(currentPageMorosos);
      } else {
        alert('Error: ' + (data.msg || 'Ocurrio un error desconocido.'));
      }
    } catch (e) {
      alert('Error de conexion al intentar notificar: ' + e.message);
    } finally {
      $btnAutoNotificacion.disabled = false;
      $btnAutoNotificacion.textContent = text;
    }
  })
