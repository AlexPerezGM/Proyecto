...
(() => {
  ...
  async function cargarResumen(id){
    ...
    ui.saldo.textContent  = (prestamoSel.saldo_total||0).toFixed(2);

    const c = prestamoSel.cuota_actual;
    if (c){
      ui.cnum.textContent   = c.numero_cuota;
      ui.cfecha.textContent = c.fecha_vencimiento;
      ui.ccap.textContent   = c.capital.toFixed(2);
      ui.cint.textContent   = c.interes.toFixed(2);
      ui.ccarg.textContent  = c.cargos.toFixed(2);
      ui.csal.textContent   = c.saldo_pendiente.toFixed(2);
    }else{
      ui.cnum.textContent = '-';
    ...
    const mora = js.mora?.mora_total || 0;
    ui.mora.textContent = mora.toFixed(2);
    const totalHoy = (prestamoSel.cuota_actual?.saldo_pendiente || 0) + mora;
