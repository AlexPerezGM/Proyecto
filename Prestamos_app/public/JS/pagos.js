document.addEventListener('DOMContentLoaded', () => {
  ...
  async function jsonFetch(body) {
    ...
    try {
      ...
      $err.hidden = false; $err.textContent = 'Respuesta no-JSON:\n' + txt.slice(0, 2000); throw e;
    }
    if (js && js.ok === false && js.msg) {
      $err.hidden = false; $err.textContent = js.msg;
    }
    return js;
  }
  ...
  $btnBuscar.addEventListener('click', async () => {
    ...
    let rows = js.data || [];
    rows = rows.filter(r => {
      const estado = (r.estado_prestamo || '').toLowerCase();
      if (!estado) return false;
      if (/(desembolso|evaluacion|cerrado|rechazado)/.test(estado)) return false;
      return /(activo|en mora)/.test(estado);
    });
    pintarTabla(rows);
