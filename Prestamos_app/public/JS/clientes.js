(() => {
  ...
  document.getElementById('numero_documento').addEventListener('blur', async () => {
    try {
      const seleccion = document.getElementById('id_tipo_documento_modal') || document.getElementById('selTipoDoc');
      const tipoId = seleccion ? String(seleccion.value || '') : '';
      if (tipoId !== '1') {
        return;
      }
  ...
  async function verificarTerceros() {
    ...
    try {
      ...
      if (res.found) {
        ...
        const p = res.data;
        if ($inpExistingId)
          $inpExistingId.value = p.id_datos_persona;
        ...
        <div id="view-datos" class="tab-pane active">
          <div class="grid-2">
            ...
            <div>
              ...
              <h4>Dirección</h4>
              <p>${c.ciudad || ''}, ${c.sector || ''}, ${c.calle || ''} , Casa No. ${c.numero_casa || ''}</p>
            </div>
            <div>
            ...
            <div>
              <h4>Documentacion</h4>
              <button class="btn btn-light" onclick="window.open('${(window.APP_BASE || '/') + 'views/docs_cliente.php?id_cliente=' + encodeURIComponent(c.id_cliente) + '&tipo=personales'}', '_blank')">Abrir documentos Personales</button>
            </div>
          </div>
        </div>
        ...
        if (tabName === 'prestamos' && id_cliente) {
          ...
          try {
            ...
            if (data.historial.length === 0) {
              html += `<tr><td colspan="9" class="text-center">No hay historial de préstamos.</td></tr>`;
            } else {
              data.historial.forEach(p => {
                ...
                const urlDocsPrestamo = (window.APP_BASE || '/') + `views/docs_cliente.php?id_cliente=${encodeURIComponent(id_cliente)}&tipo=prestamo&id_prestamo=${p.id_prestamo}`;
                        ...
                        <tr>
                          ...
                          <td>
                            <button class="btn btn-sm btn-light" 
                              onclick="window.open('${urlDocsPrestamo}', '_blank')" 
                              title="Ver documentos del préstamo">
