(() => {
  ...
  const showAsist = () => {
    if ($secEmp) $secEmp.style.display = 'none';
    if ($secNom) $secNom.style.display = 'none';
    if ($secAsist) $secAsist.style.display = '';
    $tabEmp?.classList.add('btn-light');
    $tabNom?.classList.add('btn-light');
    $tabAsist?.classList.remove('btn-light');
    try { $secAsist.scrollIntoView({ behavior: 'smooth' }); } catch (e) { }
  };

  $tabEmp?.addEventListener('click', showEmp);
  $tabNom?.addEventListener('click', showNom);
  $tabAsist?.addEventListener('click', showAsist);
  ...
  const $fechaAsist = document.getElementById('fechaAsist');
  const $btnCargarAsist = document.getElementById('btnCargarAsist');
  // En la vista los botones se llaman btnMarcarEntrada / btnMarcarSalida
  const $btnEntrada = document.getElementById('btnMarcarEntrada');
  const $btnSalida = document.getElementById('btnMarcarSalida');
  const $tbAsist = document.querySelector('#tablaAsistencia tbody');
  ...
  async function cargarAsistencia() {
    const j = await jsonFetch(API, new URLSearchParams({
      action: 'asis_list', fecha: $fechaAsist.value
    }));
    $tbAsist.innerHTML = (j.data || []).map(r => `
      <tr>
        <td>${r.nombre}</td>
        <td>${r.hora_entrada || '-'}</td>
        <td>${r.hora_salida || '-'}</td>
        <td>${r.retraso_minutos || 0}</td>
      </tr>
    `).join('');
  }
  $btnCargarAsist.addEventListener('click', cargarAsistencia);

  $btnEntrada.addEventListener('click', async () => {
    const j = await jsonFetch(API, new URLSearchParams({ action: 'asis_entrada' }));
    if (j.ok) alert('Entrada registrada.'); else alert(j.msg || 'Error');
    cargarAsistencia();
  });

  $btnSalida.addEventListener('click', async () => {
    const j = await jsonFetch(API, new URLSearchParams({ action: 'asis_salida' }));
    if (j.ok) alert('Salida registrada.'); else alert(j.msg || 'Error');
    cargarAsistencia();
  });
