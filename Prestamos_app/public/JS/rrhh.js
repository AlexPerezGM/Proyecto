document.addEventListener("DOMContentLoaded", () => {
  ...
  function resetDocsUIForCreate() {
    if (!$tipoDocEmpleados || !$archivoDoc || !$btnSubirDoc || !$boxDocs) return;
    isEditMode = false;
  ...
  document.addEventListener('click', async (e) => {
    ...
    if (ed) {
      ...
      if (!data.ok) return alert('Error al cargar datos del empleado: ' + (data.msg || ''));
      const c = data;
      const html = `
        <div class="grid-2">
          <div>
  ...
  async function abrirAjustesEmpleado(idEmpleado) {
    ...
    if ($ajusteIdEmpleado) $ajusteIdEmpleado.value = idEmpleado;
    if ($ajusteIdAjuste) $ajusteIdAjuste.value = 0;

    if ($ajusteTipo) $ajusteTipo.value = '';
    if ($ajusteIdCatalogo) {
  ...
  async function listarEmpleados() {
    ...
    if (!validarPeriodo(per))
      return alert('Seleccione un periodo valido')
    ...
    const res = await jsonFetch(API, params);
    if (!res.ok) return alert(res.msg || 'Error al buscar empleados');
    ...
    if ($msgInfoNomina) $msgInfoNomina.style.display = 'block';
  ...
  $txtSearchNomina?.addEventListener('keydown', e => {
    if (e.key === 'Enter') listarEmpleados();
  });

  $btnProcesarNomina?.addEventListener('click', async () => {
    const per = $periodo ? $periodo.value.trim() : '';
    if (!validarPeriodo(per)) return alert('periodo invalido');
    ...
    if (itemsToSave.length > 0) {
      ...
      if (!saveRes.ok) return alert("Error al guardar" + saveRes.msg);
    }
    ...
    if ($msgInfoNomina) $msgInfoNomina.style.display = 'none';
  ...
  $btnGuardar?.addEventListener('click', async () => {
    ...
    if (!validarPeriodo(per)) {
      if ($errNom) {
        $errNom.hidden = false; $errNom.textContent = 'Ingrese un periodo válido (YYYY-MM) antes de guardar.';
      }
      return;
    }
    ...
    if (!rows.length) {
      if ($errNom) {
        $errNom.hidden = false; $errNom.textContent = 'No hay datos calculados para guardar.';
      }
      return;
    }
    const fd = new FormData();
    ...
    if (!j.ok) {
      if ($errNom) {
        $errNom.hidden = false; $errNom.textContent = j.msg || 'No se pudo guardar.';
      }
      return;
    }
