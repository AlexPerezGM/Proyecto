...
CREATE TABLE documentacion_empleado(
    ...
    ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT fk_tipo_documentoC FOREIGN KEY (id_documentacion_empleado) REFERENCES cat_documentacion_empleado(id_documentacion_empleado)
)ENGINE = INNODB;
...
CREATE INDEX idx_cronograma_estado ON cronograma_cuota(estado_cuota);
...
BEGIN
    ...
    SELECT COALESCE(SUM(capital_cuota - capital_pagado), 0)
    INTO v_saldo_pendiente
    FROM cronograma_cuota
    WHERE id_prestamo = p_id_prestamo AND estado_cuota = 'Pendiente';
...
BEGIN
    ...
    SELECT porcentaje_mora INTO v_mora
    FROM config_mora
    WHERE estado = 'Activo'
