...
/**
 * Obtiene datos básicos del cliente para nombrar carpeta/archivos:
...
/**
 * Crea (si no existe) la carpeta del cliente usando el número de documento.
 * Ruta física: /uploads/clientes/{NUM_DOC_SANITIZADO} o CLI_{id_cliente} si no hay doc.
 */
...
/**
 * Devuelve el id_documentacion_cliente adecuado según el tipo de archivo (CEDULA, PASAPORTE, etc.).
 * Intenta buscar en cat_documentacion_cliente por alguna columna de texto que contenga ese nombre.
 * Si no encuentra coincidencia, usa el primer registro de la tabla como fallback.
 */
function resolve_id_documentacion_cliente(mysqli $conn, string $tipoCodigo): int {
    ...
    // Detectar columnas de texto en cat_documentacion_cliente
    $cols = [];
...
try {
    ...
    if ($action === 'upload_doc') {
        ...
        if (table_has_column($conn, 'documentacion_cliente', 'id_cliente') &&
            ...
            if ($tieneTipoCat) {
                ...
                // 🔁 Insertar o actualizar (evita Duplicate entry ...)
                $sqlIns = "
    ...
    // =============================================
    // 4. Listar documentos (lee del disco)
    // =============================================
    if ($action === 'list_docs') {
        $id = i('id_cliente', 0);
        if ($id <= 0) bad('id_cliente requerido', 400);
